library(readr)
my_data1<-read.csv("C:/Downloads/corona_test.csv")
View(my_data1)
str(my_data1)
library(dplyr)
...
#we get t = 115.32, df = 278847, p-value < 2.2e-16 which is less than 0.05.
...
summary(aov1)
#we get p<2e-16<0.05=>H0 rejected=>head_ache has impact on corona_result
...
aov4
summary(aov4)
#we get p<2e-16<0.05,so H0 rejected=>fever and sore_throat has impact on cough
...
summary(aov5)
#we get p<2e-16<0.05,so H0 rejected=>fever,sore_throat,head_Ache,shortness_of_breath and cough together have impact on corona_result
...
model<-glm(formula=corona_result~cough+fever+sore_throat+shortness_of_breath+head_ache,data=train,family="binomial")
